...
#    beat_title = models.CharField(max_length=250)
...
#====now model to handle client who wants to buy beats====#
#===the ony client detail i would collect from him;
#==email or phone number, country === just to give me some stats
#===i would also want to record the beats he has purchased previously===#
#==remember one client can purchase many beats===#
#so is that  oneToMany or ManyToMany relationship?
# ===but one beat cannot be purchased by many clients===
#==so its a oneToMany relationship from client to beats purchased===

#==let me create that model==
# one beat can only be purchased by one client, coz after purchasing it
#===its deleted from the store===#
#==so its a oneToMany relationship from client to beats purchased===
